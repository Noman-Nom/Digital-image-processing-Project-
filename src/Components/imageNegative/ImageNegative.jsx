...
const ImageNegativeAlgorithm = () => {
    ...
    const [bitDepthValue, setBitDepthValue] = useState(null)
    // console.log("bitdepth = ",  bitdepth)

    const handleBd = ()=>{

        const bd = 2**bitdepth - 1
        setBitDepthValue(bd)

            console.log("bd value = ",  bitdepth)
            
    }

    useEffect(()=>{
        console.log('Bit Depth Value:', bitDepthValue);
    ...
    const calculate = () => {
        console.log("value = ", bitDepthValue)
        const result = inputValues.map((row) => row.map((value) => bitDepthValue - value));
